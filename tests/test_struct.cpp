...
TEST(Struct, SetField) {
	...
	ASSERT_INSTR(STRUCT_NEW, 0, 0, 0);
	ASSERT_INSTR(STRUCT_SET_I, 0, 0, 3);
	ASSERT_INSTR(STRUCT_FIELD, 1, 0, 1);
	ASSERT_INSTR(STRUCT_FIELD, 1, 1, 2);
	ASSERT_INSTR(STRUCT_SET_I, 1, 3, 10);
	ASSERT_RET();
...
TEST(Struct, CustomConstructor) {
	...
	FN(1);
	ASSERT_INSTR(STRUCT_SET_L, 0, 0, 1);
	ASSERT_RET();
