...
TEST(return_value) {
	...
	ASSERT_INSTRUCTION(RET_L, 1, 0, 0);
}
...
TEST(arguments_and_return) {
	...
	ASSERT_INSTRUCTION(RET_L, 2, 0, 0);
}
...
TEST(call_arg) {
	...
	SELECT_FN(0);
	ASSERT_INSTRUCTION(MOV_LF, 0, 1, 0);
	ASSERT_INSTRUCTION(MOV_LI, 1, 2, 0);
	ASSERT_CALL(CALL_L, 1, 0, 1, 1);
	ASSERT_RET();
...
TEST(call_multiple_args) {
	...
	ASSERT_INSTRUCTION(MOV_LI, 3, 3, 0);
	ASSERT_CALL(CALL_L, 3, 0, 1, 1);
	ASSERT_RET();
...
TEST(call_return_value) {
	...
	SELECT_FN(1);
	ASSERT_INSTRUCTION(RET_I, 3, 0, 0);
}
