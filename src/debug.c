...
void pretty_print_bytecode(Bytecode *bytecode) {
	...
	#define READ_2_BYTES() \
		(cursor += 2, ((uint16_t) *(cursor - 2) << (1 << 3)) | *(cursor - 1))

	#define READ_4_BYTES() \
		(cursor += 4, \
			((uint32_t) *(cursor - 4) << (3 << 3)) | \
			((uint32_t) *(cursor - 3) << (2 << 3)) | \
			((uint32_t) *(cursor - 2) << (1 << 3)) | \
			(uint32_t) (*(cursor - 1)))

	#define READ_8_BYTES() \
		(cursor += 8, \
			((uint64_t) *(cursor - 8) << (7 << 3)) | \
			((uint64_t) *(cursor - 7) << (6 << 3)) | \
			((uint64_t) *(cursor - 6) << (5 << 3)) | \
			((uint64_t) *(cursor - 5) << (4 << 3)) | \
			((uint64_t) *(cursor - 4) << (3 << 3)) | \
			((uint64_t) *(cursor - 3) << (2 << 3)) | \
			((uint64_t) *(cursor - 2) << (1 << 3)) | \
			((uint64_t) *(cursor - 1)))
